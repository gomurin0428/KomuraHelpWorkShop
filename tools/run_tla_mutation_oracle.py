...
MUTANTS = [
    ...
    Mutant(
        ...
        "Replace error no-valid-CHM inequality with equality.",
        'phase = "Done" /\\ exitCode # 0 => outputState # "ValidChm"',
        'phase = "Done" /\\ exitCode # 0 => outputState = "ValidChm"',
        "killed",
        "Error exits must not be classified as a valid CHM.",
    ),
    Mutant(
    ...
    Mutant(
        ...
        'IF TRUE THEN',
        "killed",
        "Only the missing-required scenario may stop at collection with that error.",
    ),
    Mutant(
    ...
    Mutant(
        "M012_generated_toc_warning_removed",
        "Remove generated-TOC warning accumulation in normal metadata build.",
        'ELSE warnings \\cup {"generated toc"}',
        'ELSE warnings',
        "killed",
        "Successful compiles without contents file must preserve the generated-TOC warning.",
    ),
    Mutant(
