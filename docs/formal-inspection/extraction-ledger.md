...
| N-004 | normal | `LinkScanner` | HTML/CSS/HHC/HHK links are scanned unless `--no-link-scan` is set; supported attribute, HHC Local param, CSS import, and CSS url syntaxes are extracted as raw targets before cleanup; optional missing links warn. | Compatibility decision: keep reference-compiler style optional discovery; missing discovered files warn, and unreadable files during link scanning are treated as having no outgoing links. |
...
| B-002 | boundary | `MakeArchiveRelative` | Project-declared files outside the HHP directory are included by basename only after remediation; duplicate outside basenames warn and keep the first source. | Compatibility decision: preserve the reference behavior by reading explicit outside files while storing them by basename only. |
