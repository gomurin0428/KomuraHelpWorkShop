...
| `FileCollection` | Recursive file collection | External and fragment-only links are ignored, missing required files fail unless allowed, duplicate archive-path conflicts warn, Flat storage uses filename-only archive paths, link cycles terminate. |
