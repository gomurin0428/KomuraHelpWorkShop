---- MODULE UC059_Unsupported_MergeFiles ----
EXTENDS Integers

(*
Scenario: merge files warns and does not generate collection
...
ExpectedMetadata == {"MergeFiles:metadata-only", "Output:project/help.chm"}
ExpectedWriterTags == {"InternalStreams", "PMGL", "Uncompressed"}
ExpectedStdout == {"Banner", "Compiled", "Files"}
ExpectedStderr == {}
ExpectedWarnings == {"MERGE FILES is not generated"}
ExpectedExit == 0
...
UseCaseSpecificContract ==
  phase = "Done" =>
    /\ "MergeFiles:metadata-only" \in metadata
    /\ "MERGE FILES is not generated" \in warnings
