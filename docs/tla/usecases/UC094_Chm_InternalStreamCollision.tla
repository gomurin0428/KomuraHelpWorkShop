---- MODULE UC094_Chm_InternalStreamCollision ----
EXTENDS Integers

(*
Scenario: user archive names cannot collide with CHM internal streams
...
ExpectedCollectionTags == {"ExplicitFiles", "ReservedInternalStreamCollision"}
ExpectedArchive == {"#SYSTEM"}
ExpectedMetadata == {"Output:project/help.chm", "Title:Project Title"}
ExpectedWriterTags == {"ReservedInternalStreamCollision", "WriteFailed"}
ExpectedStdout == {}
ExpectedStderr == {"error", "internal stream collision"}
ExpectedWarnings == {}
ExpectedExit == 1
...
AllStderr == {"#SYSTEM entry is too large", "Directory entry is too large", "HHC5003", "Unable to open", "add log", "directory is too large for this compiler version", "error", "input file read error", "internal stream collision", "output file locked", "output overwrite"}
AllWarnings == {"Binary Index is not implemented", "Binary TOC is not implemented", "Full-text search index generation is not implemented", "HHC5003", "MERGE FILES is not generated", "WINDOWS custom settings are not parsed"}
...
UseCaseSpecificContract ==
  phase = "Done" =>
    /\ "ReservedInternalStreamCollision" \in collectionTags
    /\ "ReservedInternalStreamCollision" \in writerTags
    /\ "internal stream collision" \in stderr
    /\ chmCreated = FALSE
